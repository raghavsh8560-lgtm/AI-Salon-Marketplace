...
import { Menu, X, Sun, Moon, Heart, Calendar, User, Sparkles, LayoutDashboard } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16 sm:h-20">
          ...
          <div className="hidden md:flex items-center gap-8 font-semibold text-sm">
            ...
            {user && user.role === 'ADMIN' && (
              <Link
                ...
                className={`py-2 transition-colors ${
                  pathname.startsWith('/admin') ? 'text-primary' : 'text-gray-600 dark:text-gray-300 hover:text-primary'
                }`}
              >
                Admin Panel
              </Link>
            ...
            {user && user.role === 'OWNER' && (
              <Link
                ...
                className={`py-2 transition-colors ${
                  pathname.startsWith('/dashboard/owner') ? 'text-primary' : 'text-gray-600 dark:text-gray-300 hover:text-primary'
                }`}
              >
                Owner Dashboard
              </Link>
          ...
          <div className="hidden md:flex items-center gap-4">
            ...
            <button
              onClick={toggleTheme}
              className="p-2.5 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-900 rounded-xl transition-all cursor-pointer"
          ...
          <div className="flex items-center gap-2 md:hidden">
            <button
              onClick={toggleTheme}
              className="p-2 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-900 rounded-lg cursor-pointer"
              aria-label="Toggle Dark Mode"
            >
              {theme === 'light' ? <Moon className="w-5 h-5" /> : <Sun className="w-5 h-5 text-amber-400" />}
            </button>
                ...
                <Link
                  ...
                  className={`block px-3 py-2.5 rounded-xl transition-all ${
                    pathname.startsWith('/admin')
                      ? 'bg-pink-50 dark:bg-pink-950/20 text-primary'
                      : 'text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-900/40 hover:text-primary'
                  }`}
                >
                  Admin Panel
                </Link>
