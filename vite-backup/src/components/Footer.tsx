        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          <div>
            ...
            <ul className="space-y-2.5 text-sm">
              {['C-Scheme', 'Vaishali Nagar', 'Malviya Nagar', 'Raja Park', 'Mansarovar', 'Jagatpura'].map((area) => (
                <li key={area}>
                  <Link 
                    to={`/browse?area=${encodeURIComponent(area)}`}
          ...
          <div>
            ...
            <ul className="space-y-2.5 text-sm">
              {['Bridal Makeup', 'Hair Spa', 'Hair Cut', 'Facial', 'Nail Art', 'Hair Color'].map((service) => (
                <li key={service}>
                  <Link 
                    to={`/browse?service=${encodeURIComponent(service)}`}
                    className="text-gray-500 dark:text-gray-400 hover:text-primary dark:hover:text-primary transition-colors"
                  >
                    {service}
                  </Link>
          ...
          <div className="flex flex-col gap-4">
            ...
            <form onSubmit={handleSubscribe} className="flex gap-2">
              ...
              <button
                type="submit"
                className="bg-primary hover:bg-primary-hover text-white p-2.5 rounded-xl transition-all shadow-sm flex items-center justify-center flex-shrink-0 focus:outline-none active:scale-95"
              >
